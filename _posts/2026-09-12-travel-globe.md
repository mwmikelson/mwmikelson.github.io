...
/* --- NEW: The missing Flexbox rules to force side-by-side --- */
  ...
  .globe-iframe {
    ...
    border: none;
    border-radius: 8px;
    /* Matches the site's own background so the empty space around the globe
       (which is transparent inside the iframe's own document) shows the
       right color in both light and dark mode -- no JS/cross-frame theme
       detection needed, this just follows your theme's own CSS variable.
       If your theme uses a different variable name for the page background,
       swap it in here. */
    background-color: var(--global-bg-color, transparent);
  }
